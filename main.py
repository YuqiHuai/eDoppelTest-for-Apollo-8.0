...
from eDoppelTest.oracles import RecordAnalyzer
from eDoppelTest.oracles.ViolationTracker import ViolationTracker
from eDoppelTest.representation import Scenario
from eDoppelTest.representation.ad_agents import ADAgent, ADSection
from eDoppelTest.representation.pd_agents import PDAgent, PDSection
from eDoppelTest.representation.ScenarioRunner import ScenarioRunner
from eDoppelTest.representation.tc_config import TCSection
...
def eval_scenario(ind: Scenario):
    ...
    out_path_segments = ["out", f"{ind.gid:05}_{ind.cid:05}"]
    host_dir = Path(PROJECT_ROOT, *out_path_segments)
    docker_dir = Path(f"/{PROJECT_NAME}", *out_path_segments)
    if not host_dir.exists():
        host_dir.mkdir(parents=True)

    runners = srunner.run_scenario(docker_dir)
    ...
    for a, r in runners:
        ...
        record_path = Path(host_dir, a.container.container_name)
        ...
        for v in ra.get_results():
            ...
            if main_type == "collision":
                if sub_type < 100:
                    ...
                    related_data = frozenset([r.routing_str, obs_routing_map[sub_type]])
                    sub_type = "A&A"
            else:
...
def main():
    mp = get_map_service_for_map(HD_MAP)

    containers = [
        ApolloContainer(APOLLO_ROOT, f"ROUTE_{x}", PROJECT_NAME, PROJECT_ROOT, True)
        for x in range(MAX_ADC_COUNT)
    ]
    for ctn in containers:
        ctn.start_container()
        ctn.start_dreamview()
        print(f"Dreamview at {ctn.dreamview_url}")
