...
class StopSignOracle(OracleInterface):
    ...
    def __init__(self):
        ...
        self.parse_stop_sign_stop_line_string_on_map(get_map_service_for_map(HD_MAP))
    ...
    def on_new_message(self, topic: str, message, t):
        ...
        last_localization_timestamp = self.past_localization_list[
            -1
        ...
        for past_localization in self.past_localization_list[::-1]:
            ...
            if (
                last_localization_timestamp - past_localization_timestamp
                > self.ADC_INTERSECTING_STOP_LINE_MAX_LOOK_BACK_FRAMES_IN_SECOND
            ):
                break
            ...
            for past_planning in self.past_planning_list[::-1]:
                ...
                if past_planning_timestamp > past_localization_timestamp:
                    continue
                if self.is_planning_main_decision_to_stop_at_stop_sign(
                    past_planning, crossing_stop_sign_id
                ):
    ...
    def parse_stop_sign_stop_line_string_on_map(self, map_parser: MapService) -> None:
        """
        ...
        for ss_id in stop_sign_ids:
            ...
            line = LineString(
                [
                    [p.x, p.y]
                    for p in stop_sign_data.stop_line[0].segment[0].line_segment.point
                ]
            )
    ...
    def check_if_adc_intersecting_any_stop_lines(self) -> str:
        ...
        for (
            stop_sign_id,
            stop_line_string,
        ) in self.stop_sign_stop_line_string_dict.items():
            if not stop_line_string.intersection(adc_polygon).is_empty:
                return stop_sign_id
        return ""

    def is_planning_main_decision_to_stop_at_stop_sign(
        self, planning_message: ADCTrajectory, stop_sign_id: str
    ) -> bool:
        """
        ...
        if (
            ...
            == self.STOP_SIGN_VO_ID_PREFIX + stop_sign_id
        ):
            return True
