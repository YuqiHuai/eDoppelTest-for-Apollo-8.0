    ...
    def from_json(json_file_path: str) -> "Scenario":
        ...
        with open(json_file_path, "r") as fp:
            ...
            for adc in ad_section["adcs"]:
                r_ad.add_agent(
                    ADAgent(
                        adc["routing"], adc["start_s"], adc["dest_s"], adc["start_t"]
                    )
                )
            pd_section = data["pd_section"]
            r_pd = PDSection([])
            for pd in pd_section["pds"]:
                r_pd.add_agent(PDAgent(pd["cw_id"], pd["speed"], pd["start_t"]))
            tc_section = data["tc_section"]
            r_tc = TCSection(
                tc_section["initial"],
                tc_section["final"],
                tc_section["duration_g"],
                tc_section["duration_y"],
                tc_section["duration_b"],
            )
