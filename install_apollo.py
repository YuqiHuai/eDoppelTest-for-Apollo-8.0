...
def download_apollo():
    if APOLLO_ROOT.exists() and APOLLO_FLAGFILE.exists():
        logger.error("Apollo already installed")
        return
    else:
...
def compile_apollo():
    ctn = ApolloContainer(
        APOLLO_ROOT, f"{PROJECT_NAME}_installer", PROJECT_NAME, PROJECT_ROOT, True
    )
    ...
    ctn.exec("bash apollo.sh build cyber", verbose=True)
...
if __name__ == "__main__":
    ...
    logger.info(f"Installation took {round(minutes, 2)} minutes")
