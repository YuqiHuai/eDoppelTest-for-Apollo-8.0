...
from .proto_v8.modules.common_msgs.chassis_msgs.chassis_pb2 import Chassis
...
from .proto_v8.modules.common_msgs.perception_msgs.traffic_light_detection_pb2 import (
    TrafficLightDetection,
)
from .proto_v8.modules.common_msgs.planning_msgs.planning_pb2 import ADCTrajectory
from .proto_v8.modules.common_msgs.routing_msgs.routing_pb2 import RoutingRequest
...
class CyberBridge:
    ...
    def __prepare_bytes(data: bytes) -> bytes:
        ...
        shifts = [0, 8, 16, 24]
        for s in shifts:
            result += ((len(data) >> s).to_bytes(4, byteorder="big")[-1]).to_bytes(
                1, byteorder="big"
            )
        result += data
        return result
    ...
    def add_subscriber(self, channel: Channel, cb):
        ...
        data += self.__prepare_bytes(to_bytes(channel.name))
        ...
        self.subscribers[channel.name].append(cb_wrapper)
    ...
    def add_publisher(self, channel: Channel):
        ...
        """
        if channel.name in self.publishable_channel:
            return
        ...
        data += self.__prepare_bytes(to_bytes(topic_msg_type))
        self.conn.send(data)
        self.publishable_channel.add(channel.name)
    ...
    def publish(self, channel: Channel, data: bytes):
        ...
        msg += self.__prepare_bytes(to_bytes(channel.name))
